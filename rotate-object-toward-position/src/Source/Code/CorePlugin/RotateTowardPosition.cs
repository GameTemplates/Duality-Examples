...
namespace Rotation
{
    //creating a component called RotateTowardPosition that also inherit ICmpUdatable so we can implement an interface to the OnUpdate method
    public class RotateTowardPosition : Component, ICmpUpdatable
    {
        ...
        public void OnUpdate() //everything in this method is being updated every frame during runtime
        {
            ...
            if(DualityApp.Mouse.ButtonPressed(MouseButton.Left))
            {
                ...
                var clickX = DualityApp.Mouse.Pos.X;
                var clickY = DualityApp.Mouse.Pos.Y;

                //convert object position to sreen coordinates because the mouse return screen coordinates
                Vector3 objectPos = mainCamera.GetScreenCoord(new Vector2(this.GameObj.Transform.Pos.X, this.GameObj.Transform.Pos.Y));
